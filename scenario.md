...
IP
linux1
Inside host / traffic source
192.168.1.100 (primary) + secondary aliases
linux2
Outside host / traffic source
10.10.0.100 (primary) + secondary aliases
FortiGate
Firewall under test
TBD via config file
FortiAnalyzer
...
User inputs a rule count (e.g., 1000). The generator produces that many firewall policies via FortiGate REST API, distributed across a target VDOM, with four deliberate overlap/redundancy patterns embedded throughout:
...
Duplicate rules with different names — identical src/dst/service/action, different policy names and IDs. Simulates copy-paste accumulation over time.
...
All generated policies will be tagged with a consistent comment/label (e.g., LAB-TEST-2025) to make them identifiable for later log filtering and cleanup.
Rule distribution across protocols/services:
...
linux1 → linux2 (inside to outside) linux2 → linux1 (outside to inside)
Tools: Scapy (primary, Python-native) + hping3 (supplementary for TCP flag variation).
Traffic will match 60-75% of the configured rules intentionally, leaving 25-40% of rules with zero hit counts — those are the candidates for the "unused rule" detection in Phase 3.
...
Traffic is paced with configurable inter-packet delay to keep logs readable and avoid overwhelming FortiAnalyzer. Each session will log a unique 5-tuple to maximize log diversity.
Start/stop is controlled via a CLI flag or simple menu — no daemon required.
...
Also call FortiGate's native policy analysis endpoint if available in 7.6/8.0
...
On FortiGate: purge log entries where policy comment matches LAB-TEST-2025
On FortiAnalyzer: delete log entries filtered by the same tag + time window of the test
...
At the end of the exercise you will have a measurable, evidence-backed answer to: "Of N firewall rules, X% were never matched in Y days, and here is what three different optimization engines (FortiOS native, GPT-4o, Claude) each recommend doing about it."
