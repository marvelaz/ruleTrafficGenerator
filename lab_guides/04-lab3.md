...
Pass B — Structural: CIDR containment analysis
  ...
  Duplicate groups        :  ~7–13
  Shadow pairs            :  ~50–75   ← far more than ground truth (see note below)
  Subnet-overlap pairs    :  ~50–75   ← same over-count pattern
  Collapsible svc groups  :  ~5–8
...
> **Known limitation:** Pure structural CIDR analysis cannot distinguish intentional policy overlaps (planted shadows, planned redundancy) from incidental ones that emerge in dense rule sets. On a 100-rule lab pushed into a small /24-pair address pool, `phase3_traditional` typically reports 7–10× more shadow/subnet-overlap pairs than ground truth — every accidental superset relationship looks identical to a planted shadow. This over-counting is the teaching moment for Lab 4: an AI analyzer can read rule names, comments, and clustering to filter out incidental overlaps that aren't actionable.
...
Extract the counts section from both reports:

```bash
python3 -c "import json; d=json.load(open('lab_output/zero_report.json')); print(json.dumps(d['counts'], indent=2))"
python3 -c "import json; d=json.load(open('lab_output/traditional_report.json')); print(json.dumps(d['counts'], indent=2))"
```
...
> The shadow and subnet-overlap rows will show large over-counts relative to ground truth. This demonstrates why AI analysis (Lab 4) is needed: the traditional script can't tell a planted shadow from an incidental CIDR superset, so it floods the list with false positives. The AI step filters that noise.

------

> **Next Steps:** Proceed to **Lab 4** to use OpenCode + OpenRouter for interactive AI analysis of the JSON reports generated in this lab. Lab 4 reasons across the full policy set with large language models and filters the false-positive noise shown in your comparison table.