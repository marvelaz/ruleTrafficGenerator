...
3. Open `config.yaml` and replace `api_token: "REPLACE_ME"` with the token generated in Task 3. The rest of the `fortigate:` block is already pre-populated for the lab topology.
...
5. Confirm the virtual environment is active and dependencies are installed (this was done in the prerequisites — re-run only if you skipped that step):

```bash
source .venv/bin/activate
python3 -c "import rich, scapy, yaml, click; print('dependencies OK')"
```
